...
export const InputArea: React.FC<InputAreaProps> = ({ onFileSubmit, onFilesSubmit, onTextSubmit, onMixedSubmit, isLoading }) => {
  ...
  const enqueueFiles = useCallback(async (files: FileList | File[]) => {
    ...
    const accepted = list.filter(f => f.type.startsWith('image/') || f.type === 'application/pdf');
    if (!accepted.length) return;
    const dataUrls = await Promise.all(accepted.map(readFileToDataUrl));
    setFileDataUrls(prev => [...prev, ...dataUrls]);
  }, []);
  ...
  // Ajuste de alturas (más bajo en eje Y)
  const contentHeight = expanded
    ? 'min-h-[52vh] sm:min-h-[48vh] max-h-[82vh]'
    : hasText
      ? 'min-h-[22vh] sm:min-h-[20vh] max-h-[60vh]'
      : 'min-h-[96px] sm:min-h-[104px] max-h-[60vh]';
  ...
  className={`w-full max-w-none sm:max-w-4xl mx-auto bg-transparent backdrop-blur-md rounded-3xl border transition-all duration-300 ${dragOver ? 'border-violet-400/60 shadow-violet-500/20 shadow-2xl scale-[1.02]' : 'border-slate-700/40 shadow-xl'} p-3 sm:p-5 min-h-[86px] sm:min-h-[72px]`}
    >
      {/* Thumbnails first - above input on mobile */}
      {fileDataUrls.length > 0 && (
        <div className="flex gap-3 flex-wrap mb-3 p-3 bg-transparent rounded-2xl border border-slate-700/30">
  ...
  <div className="flex items-stretch gap-3 justify-center">
        ...
        <div
          className={`relative flex-1 max-w-3xl transition-all duration-300 ${contentHeight} overflow-hidden rounded-3xl bg-slate-800/30 backdrop-blur-sm border border-slate-600/50 px-4 pt-6 pb-16 sm:px-5 sm:pt-6 sm:pb-16 focus-within:border-violet-400/60 focus-within:bg-slate-800/40`}
        >
          ...
          <button
            ...
            className="absolute top-2 right-2 z-10 w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-slate-900/60 hover:bg-slate-800/70 text-slate-300 hover:text-white border border-slate-600/60 hover:border-slate-500 shadow-md flex items-center justify-center transition-all duration-200"
          >
            {expanded ? (
          ...
          {/* Barra inferior con botones dentro del cuadro */}
            ...
            <button
              ...
              disabled={isLoading}
              className="pointer-events-auto w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-slate-900/60 hover:bg-slate-800/70 text-violet-200 hover:text-white border border-slate-600/60 hover:border-violet-400/60 shadow-md flex items-center justify-center transition-all duration-200"
            ...
            <button
              ...
              className="pointer-events-auto w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-br from-violet-600 via-purple-600 to-indigo-600 hover:from-violet-500 hover:via-purple-500 hover:to-indigo-500 text-white flex items-center justify-center shadow-lg transition-all duration-200 border border-white/10"
              title="Enviar"
            >
              {isLoading ? (
                <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
              ) : (
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 transform translate-x-0.5">
          ...
          <textarea
            ...
            className="w-full resize-none bg-transparent outline-none text-slate-100 placeholder-slate-400/70 text-base sm:text-lg leading-relaxed"
            rows={hasText ? (expanded ? 9 : 5) : 3}
            disabled={isLoading}
            style={{ minHeight: hasText ? (expanded ? '48vh' : '18vh') : '96px' }}
          />
        </div>
      </div>

      {/* Footer info - centered and minimal */}
      <div className="flex items-center justify-center mt-3 text-xs text-slate-400/60">
        <span className="text-[10px] sm:text-xs">
          {fileDataUrls.length > 0 ? `${fileDataUrls.length} archivo${fileDataUrls.length > 1 ? 's' : ''}` : 'Ctrl+V para pegar imágenes'}
        </span>
      </div>
    </div>
  );
