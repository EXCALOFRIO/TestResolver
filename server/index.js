...
async function aiGenerateName(questions, userKey) {
  const modelsTry = ['gemini-3-flash-preview', 'gemini-2.5-flash-lite', 'gemini-2.5-flash', 'gemini-2.0-flash-lite'];
...
async function pingKey(apiKey) {
  try {
    ...
    // Usar un modelo estable y payload correcto
    ...
    if (isRate) return { valid: true, rateLimited: true };
    return { valid: false, rateLimited: false };
  }
}
const envKeyCache = new Map();
const ENV_CACHE_MS = 60 * 60 * 1000; // Reducir a 1 hora para mayor dinamismo
async function validateEnvKey(entry) {
  const now = Date.now();
  const c = envKeyCache.get(entry.value);
  if (c && (now - c.last) < ENV_CACHE_MS && c.valid) return true;
  const res = await pingKey(entry.value);
  envKeyCache.set(entry.value, { last: now, valid: res.valid });
  return res.valid;
}
async function buildKeyPool(maxKeys = 60) {
  ...
  for (const row of dbKeys) {
    ...
    const stale = !row.last_checked_at || (NOW - new Date(row.last_checked_at).getTime()) > STALE;
    let valid = row.last_valid;
    if (stale || !valid) {
      const res = await pingKey(row.api_key);
      valid = res.valid;
...
app.post('/api/ai/extract', auth, async (req, res) => {
  ...
  try { const key = await getUserPrimaryKey(req.user.uid); if (!key) return res.status(400).json({ error: 'USER_KEY_INVALID' }); const client = new GoogleGenAI({ apiKey: key }); const resp = await client.models.generateContent({ model: 'gemini-3-flash-preview', contents: text.slice(0, 30000) }); const raw = resp?.text?.() || resp?.text || ''; return res.json({ ok: true, raw }); } catch (e) { console.error('[ai.extract]', e); return res.status(500).json({ error: 'SERVER_ERROR' }); }
});
